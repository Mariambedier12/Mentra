...
export default function VerifyOtpClient({ emailParam }: VerifyOtpClientProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/auth/verify-success?email=${encodeURIComponent(emailParam)}`);
      } else {
          ...
          <div className="w-full lg:w-1/2 flex justify-center">
            <div className="w-full max-w-md bg-[#FAF9F7] rounded-4xl shadow-2xl p-8 md:p-12 min-h-[550px] flex flex-col justify-between">
              <div className="flex items-center gap-3 mb-8">
                ...
                <h1 className="text-3xl font-bold text-[#1E1E1E]">Verification</h1>
              </div>
              ...
              <p className="text-md text-[#1E1E1E] text-center mb-4">
                ...
                <br />
                <span className=" text-[#091A58]">{email}</span>
              </p>
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-full text-white text-lg transition cursor-pointer
                  ${isComplete ? "bg-[#091A58] hover:opacity-90" : "bg-gray-300 cursor-not-allowed"}
                `}
              >
